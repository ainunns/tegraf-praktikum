...
bool solveKnightTour(int x, int y, int move, int N, vector<vector<int>> &board) {
    if (move == N * N) {
        // Check if the last move can connect to the first move for closed tour
        int firstX = 0, firstY = 0; // Starting position
        int dx[] = { -2, -1, 1, 2, 2, 1, -1, -2 };
        int dy[] = { 1, 2, 2, 1, -1, -2, -2, -1 };
        for (int i = 0; i < 8; i++) {
...
void knightTourClosed(int N) {
    ...
    if (solveKnightTour(0, 0, 1, N, board)) {
        ...
        cout << "No closed tour solution exists." << endl;
    }
}
